...
;(function($){
  var Dialog = function(config){
    ...
    this.shadow = $('<div class="g-dialog-shadow"></div>')
    //创建弹出框
  ...
  Dialog.prototype = {
    ...
    create:function(){
      ...
      if(this.isConfig){
        ...
        //添加animate.css的动画效果(需要引入animate.css)
        if(config.animateCss){
          win.addClass("animated").addClass(config.animateCss);
        }
        if(config.needMask){
          mask.append(this.shadow);
        }
        mask.append(win);
        body.append(mask);
      }else {
